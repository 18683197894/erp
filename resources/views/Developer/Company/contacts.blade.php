...
<script>
  layui.config({
    ...
    var tab = table.render({
      ...
      ,url: '/developer/contacts'
      ,where:{_token:token,company_id:$('#company_id').val()}
      ,method:'post'
    ...
    $('.demoTable .layui-btn').on('click',function(){
    	name = $('#name').val();
    	tab.reload({where:{name:name,_token:token,company_id:$('#company_id').val()},page:{curr:1}});
    });
    //头工具栏事件
    table.on('toolbar(test-table-toolbar)', function(obj){
      ...
      switch(obj.event){
        ...
        layer.confirm('确定删除联系人: '+name+' 吗', function(index){
        	$.ajax({
          	url:'{{ url("/developer/contacts-del") }}',
          	type : 'post',
    ...
    form.on('submit(add)',function(data){
      ...
      $.ajax({
        url : '{{ url("/developer/contacts-add") }}',
        type : 'post',
        data : data,
        success : function(res)
        { 
    ...
    form.on('submit(edit)',function(data){
      ...
      data._token = token;
      $.ajax({
        url : '{{ url("/developer/contacts-edit") }}',
        type : 'post',
        data : data,
        success : function(res)
        { 
