    ...
    public function message(Request $request)
    {
    	...
    	{   
    		...
    		switch($request->type)
            {
                case 1 :
                    $data = MsgUser::select('*')
                        ->select('app_msg_user.*','app_msg.id as msg_id','app_msg.title','app_msg.type','app_msg.created_at as msg_created_at','app_msg.created_uid','sys_user.username')
                ...
                case 0 :
                    $data = MsgUser::select('*')
                        ->select('app_msg_user.*','app_msg.id as msg_id','app_msg.title','app_msg.type','app_msg.created_at as msg_created_at','sys_user.username')
                        ->join('app_msg',function($join){
                            $join->on('app_msg_user.msg_id','=','app_msg.id');
                        })
                        ->join('sys_user',function($join){
